...
contract FiberRouter is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    */
    ...
    function swapAndCrossOneInch(
        ...
        IERC20Upgradeable(fromToken).approve(swapRouter, amountIn);
    ...
    function withdrawSignedAndSwapOneInch(
        ...
        amountIn = IERC20Upgradeable(foundryToken).balanceOf(address(this)); // Actual amount received
        IERC20Upgradeable(foundryToken).approve(swapRouter, amountIn);
        (bool success, ) = address(swapRouter).call(_calldata);

        if (!success) {
            revert('SWAP_FAILED');
        }
        IERC20Upgradeable(foundryToken).transfer(to, amountOutMin);
        emit Withdraw(
            foundryToken,
            to,
            amountOutMin,
