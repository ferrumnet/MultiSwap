...
import "../common/cctp/ITokenMinter.sol";
import "../common/cctp/ITokenMessenger.sol";
import "../common/cctp/IMessageTransmitter.sol";
...
contract FiberRouter is Ownable, TokenReceivable {
    ...
    address public cctpTokenMinter;
    address public cctpTokenMessenger;
    address public cctpTokenTransmitter;

    // Target network information
    uint32 public targetNetworkDomain;
    bytes32 public targetCCTPFundManager;
    bytes32 public targetCCTPCaller;
   ...
   /**
    ...
    function initializeCCT(
        address _cctpTokenMinter,
        address _cctpTokenMessenger,
        address _cctpTokenTransmitter,
        address _usdcToken,
        uint32 _targetNetworkDomain,
        bytes32 _targetCCTPFundManager,
        bytes32 _targetCCTPCaller
    ) external onlyOwner {
        require(address(cctpTokenMinter) != address(0), "FR: TokenMinter address cannot be zero");
        require(address(cctpTokenMessenger) != address(0), "FR: TokenMessenger address cannot be zero");
        require(address(cctpTokenTransmitter) != address(0), "FR: TokenMessenger address cannot be zero");
        require(usdcToken != address(0), "FR: usdcToken address cannot be zero");
        require(targetCCTPFundManager != address(0), "FR: targetCCTPFundManager address cannot be zero");
        require(targetCCTPCaller != address(0), "FR: targetCCTPCaller address cannot be zero");
        require(targetNetworkDomain != 0, "FR: targetNetworkDomain cannot be zero");

            cctpTokenMinter = _cctpTokenMinter;
            cctpTokenMessenger = _cctpTokenMessenger;
            cctpTokenTransmitter = _cctpTokenTransmitter;
            usdcToken = _usdcToken;
            targetNetworkDomain = _targetNetworkDomain;
            targetCCTPFundManager = _targetCCTPFundManager;
            targetCCTPCaller = _targetCCTPCaller;
    }

  // Function to transfer USDC using CCTP
    function transferUSDC(uint256 amount, bytes32 mintRecipient) external {
        // Approve USDC transfer to TokenMinter
        require(
            IERC20(usdcToken).approve(cctpTokenMinter, amount),
            "Failed to approve USDC transfer"
        );

        // Mint USDC tokens on the local domain
        TokenMinter(cctpTokenMinter).mint(
            targetNetworkDomain,  // Target domain (local domain)
            bytes32(uint256(address(this))),  // USDC token on the local domain
            address(this),  // Address to receive minted tokens
            amount
        );

        // Prepare message for cross-chain transfer
        bytes memory burnMessage = TokenMessenger(cctpTokenMessenger).formatBurnMessage(
            bytes32(uint256(usdcToken)),  // USDC token on the local domain
            mintRecipient,
            amount
        );

        // Send the cross-chain transfer message
        MessageTransmitter(cctpTokenTransmitter).sendMessageWithCaller(
            targetNetworkDomain,          // Target domain
            targetCCTPFundManager,        // FundManager on the target domain
            targetCCTPCaller,              // Authorized caller on the target domainx
            burnMessage                    // Message to send
        );
  ...
  // Initiate a Circle Cross-Chain Transfer
    function initiateCCT(
        address token,
        uint256 amount,
        uint256 targetNetworkDomain,
        address targetToken
    ) internal nonReentrant {
        // Approve the CCTP contracts to spend USDC
        require(IERC20(usdc).approve(cctpTokenMessenger, amount), "Approval failed");
        ...
        uint64 depositNonce = ICCTPTokenMessenger(cctpTokenMessenger).depositForBurn(
            ...
            bytes32(uint256(cctpFundManager)),  // receiver address where the target tokens are minted
            usdc
        );
