...
### SwapTypesData
- **Purpose:** Holds data required for executing omniswaps or CCTP transfers across different blockchain networks.
- **Fields:**
  - `cctpType` (`bool`): Boolean indicating if this swap should route through CCTP
  - `multiTokenType` (`uint256`): The omnichain token standard type. 0 for regular swaps. Greater than 0 for different multichain token standard implementations. Currently only supports Axelar's ITS standard, where `multiTokenType` would equal q
  - `chainIdString` (`string`): Chain identifier in plain text format, as required by Axelar's set of contracts.
...
- **Modifiers:** `nonReentrant`
- **Parameters:**
  - `swapTypes` (SwapTypesData memory): Data containing configs for omniswap or CCTP type swaps
...
- **Purpose:** Executes a signed withdrawal combined with a swap operation through a specified router. Also has the option to call a specific multichain token standard protocol (currently only Axelar implemented)
- **Visibility:** Public, Virtual
- **Modifiers:** `nonReentrant`
- **Parameters:**
  - `swapTypes` (SwapTypesData memory): Data containing configs for omniswap or CCTP type swaps
...
- **Flow:** The function first checks the validity of the signatures and the transaction expiry. If valid, it executes the withdrawal, followed by a swap via the specified router, ensuring the output meets or exceeds the `minAmountOut`. It then checks `swapTypes` to determine if a multichain token standard is used, and executes an omniswap accordingly.
- **Effects:** Modifies the state by transferring the `foundryToken` and receiving the `targetToken` at the `to` address. Has a possibility of doing an omniswap.
...
- **Purpose:** A callback function executed by Axelar's relayers when transferring ITS tokens cross-chain. This allows execution of arbitrary logic using the passed payload.
- **Visibility:** Private
- **Parameters:**
  - `commandId` (bytes32): Unique identifier for the command.
  - `sourceChain` (string): The source chain identifier.
  - `sourceAddress` (bytes): The address on the source chain.
  - `fiberRouterCalldata` (bytes): Calldata for the FiberRouter contract.
  - `tokenId` (bytes32): Unique token identifier.
  - `token` (address): The token address.
  - `amount` (uint256): The amount of tokens to be transferred.
- **Flow:** Attempts to execute the `fiberRouterCalldata` to perform a swap and cross-router operation. If the execution fails, the function transfers the tokens to the recipient and emits an `OmniSwapFailed` event.
- **Effects:** Makes a swap attempt and transfers tokens if the swap fails.
