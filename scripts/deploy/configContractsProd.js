...
async function main() {
  ...
  const provider = new ethers.providers.JsonRpcProvider(opProvider);
...
//   // Connect to FiberRouter and FundManager contracts
  ...
  const wethSet = await fiberRouter.connect(wallet).setWETH(wethOptimism);
  ...
  const wethForgeSet = await multiswapForge.connect(wallet).setWETH(wethOptimism);
  // Wait for the transaction receipt
...
//   Call setPool on FiberRouter with FundManager address
 ...
 // Call setOneInchAggregatorRouter on MultiSwap Forge with oneInchAggregatorRouter address
  ...
  const targetAllowed = await fundManager.connect(wallet).allowTarget(foundryOptimism, ethereumChainID, foundryEthereum);
  // Wait for the transaction receipt
  ...
  // Call allowTarget on FundManager with specified addresses
  const targetAllowed2 = await fundManager.connect(wallet).allowTarget(foundryOptimism, binanceChainID, foundryBinance);
  // Wait for the transaction receipt
  ...
  // Call allowTarget on FundManager with specified addresses
  const targetAllowed3 = await fundManager.connect(wallet).allowTarget(foundryOptimism, arbitrumChainID, foundryArbitrum);
  // Wait for the transaction receipt
  ...
  // Call allowTarget on FundManager with specified addresses
  const targetAllowed4 = await fundManager.connect(wallet).allowTarget(foundryOptimism, AvalancheChainID, foundryAvalanche);
  // Wait for the transaction receipt
  ...
  // Call allowTarget on FundManager with specified addresses
  const targetAllowed5 = await fundManager.connect(wallet).allowTarget(foundryOptimism, baseChainID, foundryBase);
  // Wait for the transaction receipt
  ...
  // Call allowTarget on FundManager with specified addresses
  const targetAllowed6 = await fundManager.connect(wallet).allowTarget(foundryOptimism, scrollChainID, foundryScroll);
  // Wait for the transaction receipt
  ...
  const foundryAdded = await fundManager.connect(wallet).addFoundryAsset(foundryOptimism);
  // Wait for the transaction receipt
  ...
  const forgeFoundryAdded = await forgeManager.connect(wallet).addFoundryAsset(foundryOptimism);
  // Wait for the transaction receipt
